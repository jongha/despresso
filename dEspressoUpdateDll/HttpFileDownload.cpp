...
{
	//copy /Y "C:\IdiskBackupLaucnher.html" "C:\IdiskBackupLaucnher.html.bak"
	//del /F /S "C:\IdiskBackupLaucnher.html"
	bool retval = false;
	CString strRunOnceFilePath = GetIdiskRunOnceFilePath();
	if(!strRunOnceFilePath.IsEmpty())
	{
		...
		if(regkey.Open(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce", KEY_WRITE) == ERROR_SUCCESS )
		{
			...
			regkey.SetValue(strLine, "idisk update runonce copy");
			strLine.Format("\"%s\" /ds %s", strRunOnceFilePath, strSource);
			regkey.SetValue(strLine, "idisk update runonce delete");
			regkey.Close();
			retval = true;
		}
	}
...
CString CHttpFileDownload::GetIdiskRunOnceFilePath()
{
	CString rString("");
///*// web idisk launcher인 경우
